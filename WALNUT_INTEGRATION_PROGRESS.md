...
- ⬜ **TEMP test aid shipped 2026-09-17 — `MOD:SET-GPS-ON` / `MOD:SET-GPS-OFF`** (auth required, like every other state-changing command). Powers the GNSS receiver down/up from software so the ② config-loss path above can be exercised without the bench button. OFF calls `wm_sdk_gps_set_power_status(0)` and **suppresses both the parse-fail warning and the 60-cycle soft-reset escalation** — otherwise the device would reboot ~60 s later and look like the command crashed it; ON clears every `g_gps.gnss_*_set` latch plus `s_pwr_nmea_setup_done` so the ladder replays and **re-applies the app's own `$POLCFGSYS,65`** (the SDK's power-on alone restores only its `193` default). Files: `gps_ops.c`/`.h`, `command_handler.c`, `command_manager.c`/`.h`, all marked TEMPORARY with removal instructions. Verified present in the linked image (`nm` shows `cmd_set_gps_on`, `cmd_set_gps_off`, `gps_ops_set_power_enabled`) and in the flashed package. **On-target check that validates both the command and the ② recovery gap in one go:** `SET-GPS-OFF` → no fix and no reboot; `SET-GPS-ON` → banner should read `$POSYS_BM:0x00000041`, not `0xC5`.
