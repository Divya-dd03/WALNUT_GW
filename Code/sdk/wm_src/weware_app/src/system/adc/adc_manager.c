/**
 * @file adc_manager.c
 * @brief ADC manager - thin walnut wrapper over sdk_adc (values in volts).
 */
...
Result adc_manager_init(void)
{
    ...
    if (sdk_adc_read_vbat_voltage(&vbat_mv) != SDK_RESULT_SUCCESS)
...
Result adc_manager_get_voltage(AdcChannel channel, float *value)
{
    ...
    if (sdk_adc_read_voltage((INT32)channel, &mv) != SDK_RESULT_SUCCESS) {
...
Result adc_manager_get_vbat_voltage(float *value)
{
    ...
    if (sdk_adc_read_vbat_voltage(&mv) != SDK_RESULT_SUCCESS) {
