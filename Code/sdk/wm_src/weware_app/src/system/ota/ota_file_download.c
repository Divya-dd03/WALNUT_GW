                           ...
                           UINT32 *crc_out)
{
    ...
    size_t resp_len = 0;
    int ok = sdk_https_request(SDK_HTTPS_METHOD_POST, OTA_CHECK_VERSION_URL_PROD,
...
static int ota_download_write_simcom(UINT32 file_size, char *scratch, size_t scratch_size)
{
    ...
    if (open_ret != SDK_RESULT_SUCCESS)
    {
        ...
        LOG_ERROR("OTA: sdk_app_package_open failed - check SIMCOM app update API in sdk_simcom_ota.c");
        return 0;
    }
