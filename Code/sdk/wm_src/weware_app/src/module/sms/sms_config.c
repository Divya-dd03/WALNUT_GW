 ...
 * calls become sdk_log_error with the reference error-code name kept in the text -
...
Result sms_config_validate(const void *config)
{
    ...
    if (!sms_cfg)
    {
        sdk_log_error("ERR_SMS_CONFIG_FAILED: NULL config"); /* ERRC */
        return RESULT_INVALID_PARAM;
    }

    if (sms_cfg->format_mode != 0 && sms_cfg->format_mode != 1)
    {
        sdk_log_error("ERR_SMS_CONFIG_FAILED: Invalid format_mode: %d (must be 0 or 1)",
    ...
    {
        sdk_log_error("ERR_SMS_CONFIG_FAILED: Invalid urc_timeout_ms: %u (must be 1-10000)",
...
SmsConfig *sms_config_get_storage(void)
{
    ...
    {
        sdk_log_error("SMS module config not found");
        return NULL;
    }
