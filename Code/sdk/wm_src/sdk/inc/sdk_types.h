 ...
 *          Gateway SDK Platform Abstraction API (sdk_*).
 *
 *          This header is the single foundation included by every sdk_*.h
 *          domain header. It defines the standard return type (SdkResult) and
...
#ifndef SDK_HAS_INT64
#define SDK_HAS_INT64
...
**
** SdkResult: 0 = success, negative = failure. All sdk_* functions that do not
** follow BSD-socket conventions (see sdk_tcp.h) return this type.
******************************************************************************/
typedef INT32 SdkResult;

#define SDK_RESULT_SUCCESS          (0)   /* operation succeeded            */
#define SDK_RESULT_ERROR            (-1)  /* generic failure                */
#define SDK_RESULT_TIMEOUT          (-2)  /* operation timed out            */
#define SDK_RESULT_INVALID_PARAM    (-3)  /* bad / NULL argument            */
#define SDK_RESULT_NOT_INITIALIZED  (-4)  /* subsystem not initialised      */
#define SDK_RESULT_BUSY             (-8)  /* retry later (in progress)      */
#define SDK_RESULT_NOT_SUPPORTED    (-9)  /* vendor does not implement it   */
...
typedef enum
{
    SDK_NET_REG_NOT_REGISTERED = 0, /* not registered, not searching        */
    SDK_NET_REG_HOME           = 1, /* registered, home network             */
    SDK_NET_REG_SEARCHING      = 2, /* not registered, searching            */
    SDK_NET_REG_DENIED         = 3, /* registration denied                  */
    SDK_NET_REG_UNKNOWN        = 4, /* unknown                              */
    SDK_NET_REG_ROAMING        = 5, /* registered, roaming                  */
} SdkNetRegStatus;
...
typedef enum
{
    SDK_SIM_PRESENT = 0,   /* SIM present but not yet ready (PIN/PUK locked) */
    SDK_SIM_ABSENT  = 1,   /* no SIM detected                               */
    SDK_SIM_READY   = 2,   /* SIM ready (PIN ok)                            */
    SDK_SIM_ERROR   = 3,   /* SIM error / unknown                          */
} SdkSimStatus;

/*******************************************************************************
** SMS types
**
** The SMS operations that take a message queue (sdk_sms_read / sdk_sms_delete /
** sdk_sms_delete_all) report their outcome as one SdkSmsMessage, and incoming
** messages are reported the same way. A queue used with the SMS API must be
** created with msg_size == sizeof(SdkSmsMessage). Each received message's
** 'text' pointer is heap-owned by the caller - release it with
** sdk_sms_msg_free() once handled.
******************************************************************************/
/* Message format (sdk_sms_set_format). */
typedef enum
{
    SDK_SMS_FORMAT_PDU  = 0,   /* PDU mode  */
    SDK_SMS_FORMAT_TEXT = 1,   /* text mode */
} SdkSmsFormat;

/* Character set (sdk_sms_set_charset). */
typedef enum
{
    SDK_SMS_CHARSET_GSM  = 0,  /* GSM 7-bit default alphabet */
    SDK_SMS_CHARSET_UCS2 = 1,  /* UCS2 (16-bit)              */
    SDK_SMS_CHARSET_IRA  = 2,  /* IRA / ASCII                */
} SdkSmsCharset;

/* Storage-area selector (sdk_sms_read). */
typedef enum
{
    SDK_SMS_STORAGE_SM = 0,    /* SIM storage         */
    SDK_SMS_STORAGE_ME = 1,    /* module/phone memory */
    SDK_SMS_STORAGE_MT = 2,    /* combined            */
} SdkSmsStorage;

/* Maximum text-mode body length for sdk_sms_send(), in characters (a single
 * GSM-7 segment; longer text is rejected - no concatenation/UDH). */
#define SDK_SMS_MAX_BODY_LEN  (160u)

/* Kind of event delivered to the SMS message queue. */
typedef enum
{
    SDK_SMS_EVT_INCOMING      = 0,  /* a new message arrived                   */
    SDK_SMS_EVT_READ_RESULT   = 1,  /* result of sdk_sms_read()                */
    SDK_SMS_EVT_DELETE_RESULT = 2,  /* result of sdk_sms_delete / _delete_all()*/
} SdkSmsEventType;
 ...
 * message call sdk_sms_msg_free() to release it. It is NULL for events that
 * carry no text (delete results, or a read / incoming that failed). */
typedef struct
{
    SdkSmsEventType type;    /* which kind of event                       */
    SdkResult       status;  /* operation result (0 = ok)                 */
...
typedef struct
{
    ...
    SdkNetworkTime utc;         /* UTC date/time of the fix                 */
} SdkGpsNavData;

/*******************************************************************************
** TCP types (BSD socket conventions - see sdk_tcp.h)
******************************************************************************/
/* Socket event codes delivered to SdkTcpSocketCallback. */
#define SDK_TCP_EVENT_CONNECT   (1)
#define SDK_TCP_EVENT_RECV      (2)
#define SDK_TCP_EVENT_CLOSE     (3)
#define SDK_TCP_EVENT_ERROR     (4)

/* Address family and socket type for sdk_tcp_socket_create_with_callback(), so
 * app code does not have to pull in the lwIP headers just to name them. Values
 * match lwip/sockets.h. */
#define SDK_AF_INET             (2)
#define SDK_SOCK_STREAM         (1)

/* Async socket event callback: fd is the socket, event is one of the
 * SDK_TCP_EVENT_* codes. arg is reserved and always NULL - the create call
 * takes no user pointer, so key per-connection context off fd. */
typedef void (*SdkTcpSocketCallback)(int fd, int event, void *arg);

/*******************************************************************************
** HTTPS types
**
** The HTTPS client runs on top of the TCP/IP stack, so a PDP context must be up
** (see sdk_network.h) before a request can succeed. Requests are driven through
 ...
 * 0..SDK_HTTPS_SESSION_MAX-1, plus SDK_HTTPS_DOWNLOAD_INDEX. */
#define SDK_HTTPS_SESSION_MAX       (2u)

/* Session reserved for the sdk_https_download_* file-transfer calls, which take
 * no index of their own. Point it at the file with sdk_https_set_params(). */
#define SDK_HTTPS_DOWNLOAD_INDEX    (SDK_HTTPS_SESSION_MAX)
...
#define SDK_HTTPS_URL_MAX           (256u)
#define SDK_HTTPS_HEADER_MAX        (512u)
#define SDK_HTTPS_CONTENT_TYPE_MAX  (64u)

/* Most body bytes a single sdk_https_read() can return. A larger body is not
 ...
 * over - sdk_https.c asserts the relationship at compile time and explains it. */
#define SDK_HTTPS_RESP_BUF_SIZE     (4608u)

/* Request timeout applied when sdk_https_set_params() is given 0, in seconds. */
#define SDK_HTTPS_DEFAULT_TIMEOUT   (30u)

/* Delivery mode for sdk_https_init(). */
typedef enum
{
    SDK_HTTPS_MODE_SYNC  = 0,  /* sdk_https_action() blocks until the response
                                * headers are in; no queue is used            */
    SDK_HTTPS_MODE_ASYNC = 1,  /* sdk_https_action() returns immediately and a
...
typedef enum
{
    SDK_HTTPS_EVT_ACTION_DONE = 0,  /* an sdk_https_action() request finished */
} SdkHttpsEventType;

/* One event received from the HTTPS message queue. It reports the outcome only;
 * the body stays on the session, so read it with sdk_https_read() once this
 * arrives.
 *
...
** (see sdk_network.h) before sdk_mqtt_connect() can succeed. Exactly one client
** is supported at a time - see sdk_mqtt.h.
******************************************************************************/
/* Delivery guarantee used for publish and subscribe. */
typedef enum
{
    SDK_MQTT_QOS0 = 0,   /* at most once  - fire and forget, no ack          */
    SDK_MQTT_QOS1 = 1,   /* at least once - acknowledged, may be duplicated  */
    SDK_MQTT_QOS2 = 2,   /* exactly once  - two-phase handshake              */
} SdkMqttQos;

/* Field bounds for SdkMqttConfig. The broker-identity strings are copied into
 * SDK-owned storage by sdk_mqtt_config(), so anything longer is rejected. */
#define SDK_MQTT_HOST_MAX       (64u)   /* hostname or dotted-decimal IP     */
#define SDK_MQTT_PORT_MAX       (8u)    /* port as a decimal string          */
#define SDK_MQTT_CLIENT_ID_MAX  (64u)   /* MQTT client identifier            */
#define SDK_MQTT_USER_MAX       (64u)   /* user name                         */
#define SDK_MQTT_PASS_MAX       (64u)   /* password                          */
...
#define SDK_MQTT_TOPIC_MAX      (100u)

/* Largest payload a single sdk_mqtt_publish() can carry, in bytes. */
#define SDK_MQTT_PAYLOAD_MAX    (4096u)

/* Broker identity and session parameters for sdk_mqtt_config().
 *
...
/* One message delivered to an SdkMqttMsgCallback.
 *
...
typedef struct
{
    ...
    SdkMqttQos  qos;          /* QoS the message was published with            */
    BOOL        retained;     /* TRUE if the broker replayed a retained message*/
} SdkMqttMessage;

/* Received-message callback, registered per topic by sdk_mqtt_subscribe().
 * Invoked from the MQTT client's own keep-alive/receive task, so it must not
 * block: hand anything slow to an application task. Calling sdk_mqtt_publish()
 * from here is safe but will stall message reception while it transmits. */
typedef void (*SdkMqttMsgCallback)(const SdkMqttMessage *msg);

/*******************************************************************************
** OTA types
******************************************************************************/
/* Which staged image an OTA call operates on. Both stage on the internal C:
 * volume; retrieve the exact path for a type with sdk_ota_get_image_path(). The
...
typedef enum
{
    SDK_OTA_IMAGE_APP    = 0,  /* customer application image (C:/customer_app.bin) */
    SDK_OTA_IMAGE_KERNEL = 1,  /* kernel delta patch         (C:/system_patch.bin) */
} SdkOtaImageType;

/* Buffer bound for the version strings returned by sdk_ota_get_app_version() and
 * sdk_ota_get_sdk_version(), including the terminating NUL. Sized to the
 * underlying WM_CUS_APP_VERSION / WM_SDK_VERSION storage. */
#define SDK_OTA_VERSION_MAX     (100u)

/* Length of a SHA-256 digest in lowercase hex, excluding the terminating NUL.
 * sdk_ota_verify_image() requires exactly this many characters. */
#define SDK_OTA_SHA256_HEX_LEN  (64u)
 ...
 * pointer registered with sdk_uart_set_rx_callback(). Keep the handler short. */
typedef void (*SdkUartRxCallback)(UINT32 port, const UINT8 *data, UINT32 len, void *arg);
...
typedef enum
{
    SDK_I2C_SPEED_STANDARD = 0,   /* 100 kHz                                */
    SDK_I2C_SPEED_FAST     = 1,   /* 400 kHz                                */
} SdkI2cSpeed;
 ...
 * can be read and updated at runtime via sdk_storage_cred_read/write(). */
typedef enum
{
    SDK_STORAGE_CRED_ROOT_CA     = 0,  /* server / root CA certificate (PEM) */
    SDK_STORAGE_CRED_CLIENT_CERT = 1,  /* client certificate (PEM)           */
    SDK_STORAGE_CRED_CLIENT_KEY  = 2,  /* client private key (PEM)           */
} SdkStorageCredential;
...
#define SDK_STORAGE_CRED_MAX_SIZE   (2048u)
