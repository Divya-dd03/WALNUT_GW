...
#ifndef __WM_SDK_SMS_H__
#define __WM_SDK_SMS_H__
 ...
 * wm_SdkSmsMessage (see wm_sdk_types.h) to the supplied queue; incoming messages are
 * reported the same way as WM_SDK_SMS_EVT_INCOMING events. A queue used with these
 * functions must be created for a message size of sizeof(wm_SdkSmsMessage). A NULL
 ...
 * message drained from the queue, release its text with wm_sdk_sms_msg_free().
 */
 ...
 * @param  msg  message previously received via wm_sdk_msgq_recv().
 */
void wm_sdk_sms_msg_free(wm_SdkSmsMessage *msg);
 ...
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_set_format(UINT8 format);
 ...
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_set_charset(UINT8 charset);
 ...
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_set_new_msg_ind(UINT8 mode, UINT8 mt, UINT8 bm, UINT8 ds, UINT8 bfr);
 ...
 * @param  text    message body, NUL-terminated; up to WM_SDK_SMS_MAX_BODY_LEN
 *                 characters.
 * @return wm_SdkResult - 0 success; WM_SDK_RESULT_INVALID_PARAM on a NULL or oversized
 *                     argument; WM_SDK_RESULT_BUSY if the modem AT channel could
 *                     not be claimed for the send (the message was not sent -
 *                     retry); negative on failure.
 *
 * @note   Holds the modem AT channel for the duration of the send (typically
 *         under a second, occasionally a few). Other wm_sdk_* calls that issue AT
 *         may return WM_SDK_RESULT_BUSY while a send is in flight. This is
 *         required: the AT+CMGS text window swallows anything else written to
 *         the channel into the message body.
 */
wm_SdkResult wm_sdk_sms_send(const char *number, const char *text);
 ...
 * @return wm_SdkResult - 0 success; WM_SDK_RESULT_INVALID_PARAM on a NULL @p used /
 *                     @p total; WM_SDK_RESULT_ERROR if the module is not ready.
 */
wm_SdkResult wm_sdk_sms_get_storage_status(char *name, UINT32 name_size, UINT32 *used, UINT32 *total);

/**
 * @brief  Read a stored SMS by index; the outcome is delivered to @p msgq as an
 *         wm_SdkSmsMessage (type WM_SDK_SMS_EVT_READ_RESULT, with the message text on
 *         success).
 ...
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_read(UINT8 storage, UINT32 index, void *msgq);

/**
 * @brief  Delete a stored SMS by index; the outcome is delivered to @p msgq as
 *         an wm_SdkSmsMessage (type WM_SDK_SMS_EVT_DELETE_RESULT).
 * @param  index  message index to delete.
 * @param  msgq   queue to receive the result (may be NULL).
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_delete(UINT32 index, void *msgq);

/**
 * @brief  Delete all stored SMS; the outcome is delivered to @p msgq as an
 *         wm_SdkSmsMessage (type WM_SDK_SMS_EVT_DELETE_RESULT, index -1).
 * @param  msgq  queue to receive the result (may be NULL).
 * @return wm_SdkResult - 0 success; negative on failure.
 */
wm_SdkResult wm_sdk_sms_delete_all(void *msgq);
 ...
 * @param  msg_count  [out] number of wm_SdkSmsMessage events pending.
 * @return wm_SdkResult - 0 success; WM_SDK_RESULT_INVALID_PARAM on NULL argument.
 */
wm_SdkResult wm_sdk_sms_msgq_poll(void *msgq, UINT32 *msg_count);

#ifdef __cplusplus
}
#endif

#endif /* __WM_SDK_SMS_H__ */
