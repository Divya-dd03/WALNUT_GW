...
typedef void (*drvLcdTransferDoneCb)(void *);
/**
...
#define LCD_GPIO_A0   (57)
#define LCD_GPIO_RST  (56)
#define LCD_GPIO_CS   (34)

#define LCD_CMD_PRE   (0x1000)
#define LCD_DATA_PRE   (0x2000)
#define LCD_DELAY_US_PRE   (0x3000)
#define LCD_DELAY_MS_PRE   (0x4000)

#define LCD_CMD(x)      (uint32_t)(LCD_CMD_PRE  | (x&0xff))
#define LCD_DATA(x)     (uint32_t)(LCD_DATA_PRE | (x&0xff))
#define LCD_DELAY_US(x) (uint32_t)(LCD_DELAY_US_PRE | (x&0xff))
#define LCD_DELAY_MS(x) (uint32_t)(LCD_DELAY_MS_PRE | (x&0xff))
/**
 ...
 */
static inline drvLcdDirection_t drvLcdDirCombine(drvLcdDirection_t first, drvLcdDirection_t second) { return (drvLcdDirection_t)(first ^ second); }
 ...
 *
 * \param name      LCD driver enum name
 * \return
 *      - LCD instance
 *      - NULL on invalid parameter
 */
drvLcd_t *drvLcdGetByname(drvLcdName_e name);
 ...
 * \return
 *      - NOT NULL success open return device instance
 *      - NULL open panel false on invalid parameter, or invalid panel
 */
drvLcd_t* drvLcdOpen( drvLcdMoudleID_t id, drvLcdConfig_t *cfg);
 ...
 */
void drvLcdSetBackLightLevel(drvLcd_t *d, uint8_t level);
 ...
 *
 */
bool drvLcdBlend(const drvLcdLayers_t *layers, void *buf, int stride);

/**
 * \brief wait on-going data transfer done
 */
void drvLcdWaitTransferDone(drvLcd_t *d);
void drvLcdWriteCmd(drvLcd_t *d, uint16_t cmd);
void drvLcdWriteData(drvLcd_t *d, uint16_t data);

/**
 * \brief open LCD instance without init LCD
 *
 * When it is the first time to open the LCD, it will probe the panel, but not send lcd init table
 *
 * After open, pixel data can be send to panel. However, unless back light
 * id enabled, picture can't be seen on panel.
 *
 ...
 */
bool drvLcdOpenV2NotInit(drvLcd_t *d, drvLcdMoudleID_t id);

bool drvLcdBlockTransfer(drvLcd_t *d , const void *dataBufferWin, uint32_t data_size, bool sync, drvLcdTransferDoneCb funcCb);
bool drvLcdBlockTransfer_axs(drvLcd_t *d , const void *dataBufferWin, uint32_t data_size, bool mode, drvLcdTransferDoneCb funcCb);
drvLcdMoudleID_t drvLcdGetIdByName(const char* lcd_name);
OSI_EXTERN_C_END
#endif
